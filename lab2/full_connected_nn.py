...
class FullConnectedNN:
    ...
    def init_nn(self):
        ...
        self.correct_prediction = tf.equal(tf.argmax(self.output_logits, 1), tf.argmax(self.nn_out, 1), name='correct_prediction')
    ...
    def init_hidden_layers(self):
        units = self.units
        last_layout = self.nn_in

        for hl in range(len(units)):
            last_layout = self.full_connected_layer(
                last_layout,
                units[hl],
                f'{HIDDEN_FC_LAYER_WEIGHT}{hl}'
            )

        self.output_logits = self.full_connected_layer(last_layout, self.out_size, OUT_FC_LAYER_WEIGHT, output=True)
    ...
    def init_loss(self):
        ...
        if self.use_l1_reg:
            ...
            l1_penalty = self.l1_scale * np.sum(wrap_loss(weights), axis=0)

        if self.use_l2_reg:
            wrap_loss = np.vectorize(lambda a: tf.nn.l2_loss(a))
            l2_penalty = self.l2_scale * np.sum(wrap_loss(weights), axis=0)
    ...
    def full_connected_layer(self, x, num_units, name, output=False):
